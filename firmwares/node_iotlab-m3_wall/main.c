...
void _read_illuminance(phydat_t * illuminance)
{
    if ((light == NULL)) {
        puts("Unable to find sensors");
        return;
    }
    saul_reg_read(light, illuminance);
}
...
int main(void)
{
    ...
    light = saul_reg_find_type(SAUL_SENSE_LIGHT);
    if ((light == NULL)) {
        puts("Unable to find sensors");
        return -1;
    }
