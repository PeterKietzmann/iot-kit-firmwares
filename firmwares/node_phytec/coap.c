...
#define APPLICATION_NAME "Phytec"
...
extern void _read_illuminance(uint32_t * illuminance);
extern void _read_humidity(uint16_t * humidity);
...
static int handle_get_illuminance(coap_rw_buffer_t *scratch,
                               const coap_packet_t *inpkt,
                               coap_packet_t *outpkt,
                               uint8_t id_hi, uint8_t id_lo);

static int handle_get_humidity(coap_rw_buffer_t *scratch,
                               const coap_packet_t *inpkt,
                               coap_packet_t *outpkt,
                               uint8_t id_hi, uint8_t id_lo);
...
static const coap_endpoint_path_t path_illuminance =
        { 1, { "illuminance" } };

static const coap_endpoint_path_t path_humidity =
        { 1, { "humidity" } };
...
const coap_endpoint_t endpoints[] =
{
    ...
    { COAP_METHOD_GET,	handle_get_illuminance,
      &path_illuminance,   "ct=0"  },
    { COAP_METHOD_GET,	handle_get_humidity,
      &path_humidity,   "ct=0"  },
                                  ...
                                  uint8_t id_hi, uint8_t id_lo)
{
    ...
    sprintf(temperature, "%d.%d°C", temp / 100, (temp % 100) / 10);
                               ...
                               uint8_t id_hi, uint8_t id_lo)
{
    ...
    //sprintf(illuminance, "%lu.%dhPa",
     //       (unsigned long)ill / 100, (int)ill % 100);

    memcpy(response, illuminance, strlen(illuminance));

    return coap_make_response(scratch, outpkt, (const uint8_t *)response, strlen(illuminance),
                              id_hi, id_lo, &inpkt->tok, COAP_RSPCODE_CONTENT,
                              COAP_CONTENTTYPE_TEXT_PLAIN);
}
